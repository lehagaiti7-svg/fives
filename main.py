...
async def open_chat_and_fetch_messages(account: str, chat_name: str) -> dict:
    ...
    try:
        ...
        # Парсим сообщения по реальным классам MAX (messageWrapper, bubble, author, etc.)
        result = await page.evaluate("""() => {
            ...
            allNodes.forEach(node => {
                ...
                // Реакции (парсим ПЕРВЫМИ чтобы потом исключить из текста)
                const reactions = [];
                const reactionEls = node.querySelectorAll('[class*="reaction"]:not([class*="reactions"])');
                reactionEls.forEach(r => {
                    const t = (r.innerText || '').trim();
                    if (t && t.length < 30) {
                        // эмодзи = всё кроме цифр и пробелов, счётчик = цифры
                        const countMatch = t.match(/([0-9][0-9.,KkКк]*)\\s*$/);
                        const count = countMatch ? countMatch[1] : '';
                        let emoji = countMatch ? t.slice(0, countMatch.index).trim() : t;
                        if (!emoji) emoji = '👍';
                        const isActive = /reaction--active|active/i.test(r.className || '');
                        reactions.push({ emoji: emoji, count: count, active: isActive });
                    }
                });

                // Текст — клонируем bubble и вырезаем служебные элементы
                let text = '';
                const textEl = bubble.querySelector('[class*="messageText"]');
                if (textEl) {
                    text = (textEl.innerText || '').trim();
                } else {
                    // Клонируем и удаляем реакции, время, аватары
                    const clone = bubble.cloneNode(true);
                    clone.querySelectorAll('[class*="reaction"], [class*="time"], [class*="Time"], [class*="author"], [class*="avatar"], [class*="Avatar"], button').forEach(el => el.remove());
                    text = (clone.innerText || '').trim();
                }
                if (authorName && text.startsWith(authorName)) {
                    text = text.slice(authorName.length).trim();
                }
                // Финальная зачистка: убираем время в конце если просочилось
...
async def api_debug_messages(account: str):
    ...
    try:
        info = await page.evaluate("""() => {
            const result = { relevantClasses: [], sampleMessage: null, dateSeparators: [] };

            // Все классы относящиеся к сообщениям
            const classSet = new Set();
            document.querySelectorAll('[class]').forEach(el => {
                const cls = typeof el.className === 'string' ? el.className : '';
                cls.split(/\\s+/).forEach(c => {
                    if (/message|bubble|sender|author|reaction|voice|audio|date|day|sticker|attach|reply/i.test(c)) {
                        classSet.add(c);
                    }
                });
            });
            result.relevantClasses = Array.from(classSet).slice(0, 80);

            // Пример одного сообщения - первый который найдём
            const msgEl = document.querySelector('[class*="message"]:not([class*="messageInput"]):not([class*="messageList"]):not([class*="messageBar"])');
            if (msgEl) {
                result.sampleMessage = {
                    className: msgEl.className,
                    outerHTML: msgEl.outerHTML.slice(0, 2000),
                };
            }

            // Разделители дат
            const allEls = document.querySelectorAll('[class*="date"], [class*="day"], [class*="divider"], [class*="separator"]');
            allEls.forEach(el => {
                const text = (el.innerText || '').trim();
                if (text && text.length < 50) {
                    result.dateSeparators.push({
                        className: typeof el.className === 'string' ? el.className : '',
                        text: text
                    });
                }
            });
            result.dateSeparators = result.dateSeparators.slice(0, 10);
...
async def api_send_message(account: str, chat_name: str, request: Request, user: dict = None):
    ...
    try:
        ...
        input_selectors = [
            ...
            'textarea',
            'input[type="text"][placeholder*="ообщение"]',
        ...
        for sel in input_selectors:
            try:
                ...
                if el:
                    await el.click()
                    await el.fill(text) if sel == 'textarea' else await page.keyboard.type(text)
                    input_found = True
                    break
            except Exception:
                continue
...
async def api_load_older(account: str, chat_name: str, user: dict = None):
    ...
    try:
        # Прокручиваем контейнер сообщений до самого верха несколько раз
        for _ in range(6):
            await page.evaluate("""() => {
                // ищем контейнер сообщений
                const containers = document.querySelectorAll('[class*="conversation"], [class*="messages"], [class*="messageList"], [class*="scrollList"]');
        ...
        # После прокрутки парсим то, что теперь в DOM
        return await open_chat_and_fetch_messages(account, chat_name)
    except HTTPException:
        raise
    except Exception as e:
