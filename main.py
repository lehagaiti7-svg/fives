...
MESSAGES_PARSER_JS = """() => {
            ...
            allNodes.forEach(node => {
                ...
                // Реакции — ТОЛЬКО кнопки <button class="reaction"> (не контейнер "reactions")
                // Внутри: animoji (canvas-эмодзи, не извлекается) + span.counter (число)
                const reactions = [];
                const reactionBtns = node.querySelectorAll('button[class*="reaction"]');
                reactionBtns.forEach(btn => {
                    ...
                    // эмодзи в canvas — не достать, используем 👍 как заглушку
